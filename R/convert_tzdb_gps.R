...
#' @author Alex Dowlin and Sean Rohan
#' @return A data frame containing the columns "Date", "X", and "Y" from the Timezero database.
#' @examples
#' \dontrun{
#' # Path to a valid TimeZero database (.tzdb) file
#' path <- system.file("exdata", "convert_tzdb_gps", "OwnShipRecorder.tzdb", package = "GAPsurvey")
#'
#' Times in Alaska Time using "MM/DD/YYYY HH:MM:SS" character format or a POSIXct object
#' convert_tzdb_gps(path_tzdb = path,
#'                   output_file = NULL,
#'                   vessel = 176,
#'                   cruise = 202401,
#'                   haul = 22,
...
convert_tzdb_gps <- function(path_tzdb = NULL,
  ...
  if(is.null(path_tzdb)) {
    message("convert_tzdb_gps: path_tzdb is NULL. Select a .tzdb file.")
    nmea_strings <-
      choose.files(
        default = "*.tzdb",
        caption = "Select .tzdb file(s)",
        multi = TRUE,
        filters = matrix(c("TimeZero Database (.tzdb)", "*.tzdb"),
                         ncol = 2)
      )
...
#' @param x Numeric degree decimal minute coordinates
#' @author Alex Dowlin and Sean Rohan
#' @noRd
